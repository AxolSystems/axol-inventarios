...
class EntityModel {
  ...
  /// del parámetro.
  EntityModel.setName(EntityModel entity, this.entityName)
      : uuid = entity.uuid,
  ...
  static Map<String, dynamic> propsToMap(List<PropertyModel> props) {
    ...
    if (props.isNotEmpty) {
      for (PropertyModel prop in props) {
        map[prop.key] = {
          PropertyModel.dataType: PropertyModel.getIntToProp(prop.propertyType),
          PropertyModel.propName: prop.name
        };
      }
      /*for (PropertyModel prop in props) {
        map[prop.key] =
